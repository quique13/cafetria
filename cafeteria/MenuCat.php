...
$consulta_menu=mysqli_query($conexion,"select cp.*, p.descripcion as producto,p.imagen_producto as imagen,p.precio from categoria_producto as cp
...
if($row_menu=mysqli_num_rows($consulta_menu)>=1)
{
	$cadena='
...
while($row_menu=mysqli_fetch_array($consulta_menu))
{
	...
	$cadena.='<td width="110" height="140" onMouseOver="this.style.background=\'#088A29\';" onMouseOut="this.style.background=\'\';" >
	<center><img src="./rrhh/imagenes_cafe/'.$row_menu['imagen'].'" width="40" height="40" 
	onClick="javascript:  Compra(\''.$row_menu['precio'].'\',\''.$row_menu['id_categoria'].'\',\''.$total.'\',\'mas\',
	\''.$row_menu['id_producto'].'\') ;" style= "cursor:pointer;" title="Agregar producto"><br>'.utf8_encode($row_menu['producto']).'
	<br>Q'.$row_menu['precio'].'</center><button  type="button" class=" btn btn-danger btn-xs pull-right '.$class.'" title="Eliminar producto" 
	onClick="javascript:  Compra(\''.$row_menu['precio'].'\',\''.$row_menu['id_categoria'].'\',\''.$total.'\',\'menos\',\''.$row_menu['id_producto'].'\');"><span class="glyphicon glyphicon-trash"></span></button>';
	...
	<input type="hidden" name="nombre" id="producto" value="'.utf8_encode($row_menu['producto']) .'" size="2"/> </td>';
}
	$cadena.='</tr></table>';
	...
	while($row_cat=mysqli_fetch_array($consulta_cat))
	{
	...
	">
	<td ><center><img src="./rrhh/imagenes_cafe/'.$row_cat['imagen_cat'].'" width="50" height="50"><br>'.utf8_encode($row_cat['descripcion']).'
	</center> </td></tr>
	';
	}
